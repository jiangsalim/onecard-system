...
def attendance_report(request):
    ...
    if not has_filter:
        cache_key = make_key('attendance_report', today)
    ...
    if has_filter:
        ...
        total = get_or_set(
            make_key('total_students'),
            lambda: Student.objects.filter(status='active').count(),
            timeout=300
        )
    ...
    if not has_filter:
        cache.set(make_key('attendance_report', today), response, 120)
